...
description: >
  ...
  prompt caching, Test Impact Analysis y métricas de costo.
  ...
  "presupuesto IA", o pida generar/reparar pruebas Playwright con un agente.
...
| Resultados | `results/playwright-results.json`, `CLASIF_*.json`, output de `npx playwright test` |
...
- **Prioridad 5 — Resultados del fallo** (classify/heal): pedir `results/playwright-results.json`
  o el output exacto. Nunca healing sin error real.
...
CONTEXTO IA DETECTADO:
  ...
  SALIDA:         <PLAN_X.md | T_X.spec.ts | CLASIF_X.json | HEAL_X.md>
...
|---|---|---|
| `/pw-ia:plan` | Requisito → `PLAN_<FEATURE>.md` (escenarios, riesgos, precondiciones, datos, expected, contrato testid, tags). Sin código | económico |
| `/pw-ia:generate` | `PLAN_*.md` → `T_*.spec.ts` siguiendo `playwright-skill` con contexto mínimo | económico |
| `/pw-ia:classify` | Corre `scripts/classify-failures.mjs` → `CLASIF_<NOMBRE>.json` + resumen | ninguno |
...
Escribe `PLAN_*.md`, `HEAL_*.md`, prompts de Healer, `.env.ai.example`, pipelines
